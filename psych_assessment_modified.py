...
SCENE_IMG_Q1 = img_to_b64(pathlib.Path(__file__).parent / "wide_interior_pov_scene_looks_like_a_realistic_cg.png")
SCENE_IMG_Q2 = img_to_b64(pathlib.Path(__file__).parent / "image 2.png")
SCENE_IMG_Q3 = img_to_b64(pathlib.Path(__file__).parent / "image 3.png")

# 兼容旧引用
ORIGINAL_SCENE_IMG = SCENE_IMG_Q1
ADAPTED_SCENE_IMG  = SCENE_IMG_Q3  # 治愈图仍用第三张
...
QUESTIONS = [
    {
        "scene": {
            ...
            "emoji": "🧩",
            "bg": "original",   # 用 ORIGINAL_SCENE_IMG
        },
    ...
    },
    {
        "scene": {
            ...
            "emoji": "🗣️",
            "bg": "adapted",    # 用 ADAPTED_SCENE_IMG（治愈图）
        },
...
def page_assessment():
    ...
    # 背景图：第三题用治愈图，其余用原图
    bg_img = ADAPTED_SCENE_IMG if scene["bg"] == "adapted" else (BG_DATA if BG_DATA else "")
    if bg_img:
        st.markdown(f"""
        <style>
        [data-testid="stAppViewContainer"] {{
...
def page_adjustment():
    ...
    # 左边"调整前"显示第三题的场景图，右边"调整后"显示治愈图
    SCENE_IMGS = [SCENE_IMG_Q1, SCENE_IMG_Q2, SCENE_IMG_Q3]
    last_q_img = SCENE_IMG_Q3  # 第三题场景图作为"调整前"
    healed_img = img_to_b64(pathlib.Path(__file__).parent / "wide_cozy_sunlit_interior_scene_viewed_in_a_vr_ro.png")
    ...
    with col_before:
        ...
        <div style="background:rgba(40,10,10,0.5);border:1px solid #6a1a1a;border-radius:12px;padding:1.2rem;">
          ...
          <img src="data:image/png;base64,{last_q_img}" style="width:100%;border-radius:10px;margin-bottom:0.8rem;" />
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_after:
        ...
        <div style="background:rgba(10,30,10,0.5);border:1px solid #1a6a2a;border-radius:12px;padding:1.2rem;
          ...
          <img src="data:image/png;base64,{healed_img}" style="width:100%;border-radius:10px;margin-bottom:0.8rem;" />
        </div>
        """, unsafe_allow_html=True)

    st.markdown("<div style='height:1.5rem'></div>", unsafe_allow_html=True)

    # 鸡汤文字 — 对应第三题（被当众否定场景）的A/B/C答案
    HEALING_MESSAGES = {
        "A": {
    ...
    msg = HEALING_MESSAGES.get(sc.get("answer_letter", "B"), HEALING_MESSAGES["B"])

    st.markdown(f"""
    <div style="background:linear-gradient(135deg,rgba(10,30,55,0.92),rgba(6,20,40,0.95));
                border:1px solid rgba(77,195,255,0.3);border-left:4px solid #4dc3ff;
                border-radius:14px;padding:1.8rem 2rem;margin-bottom:1.2rem;
                box-shadow:0 0 30px rgba(26,122,212,0.15);">
      <div style="font-size:0.72rem;color:#4dc3ff;letter-spacing:0.14em;
                  font-family:'Share Tech Mono',monospace;margin-bottom:0.7rem;">
        ✦ A MESSAGE FOR YOU
      </div>
      <div style="font-size:1.1rem;font-weight:700;color:#e8f4ff;margin-bottom:0.8rem;line-height:1.4;">
        {msg['title']}
      </div>
      <p style="color:#a8ccee;font-size:0.92rem;line-height:1.8;margin:0;">
        {msg['body']}
      </p>
    </div>
    """, unsafe_allow_html=True)
